<?php
	...
	if(isset($_POST['supprimer_sujet'])) {
		...
		header('Location: DiagHealth_forum.php');
...
<html>
	...
	<body id="body">
		<div id="overlay_sujet">
			<form method="post">
				<div id="popup_sujet">
					<h2 class="popup_titre">Suppression du sujet</h2>
					<p class="popup_msg">Voulez-vous vraiment supprimer ce sujet ?</p>
					<div align="center">
						<input type="submit" name="supprimer_sujet" class="popup_lien" value="Supprimer">
						<a class="popup_lien" onclick="out('overlay_sujet')">Annuler</a>
					</div>
				</div>
			</form>
		</div>
		<h1>Sujet : <?php echo $titre['titre']; ?></h1>
		<form method="post">
					...
					if ($dataReponse['sujet'] == $_GET['id_sujet']) {
						...
						<tr class="reponse_tr">
							<td class="auteur_reponse">
								...
								echo '<br />';
								echo 'Le ', $jour , '/' , $mois , '/' , $annee , ' à ' , $heure , ':' , $minute, ' ', $dataReponse['modification']; ?>
							</td>
							...
							<td class="gestion_tr" >
								<?php if($_SESSION['role'] == 'Administrateur' || $auteur['idUtilisateur'] == $_SESSION['idUtilisateur']) {
									...
									<div class="option" id="<?=$dataReponse['idReponse']?>_lecture_btn">
										<a class="modifier" onclick="over('<?=$dataReponse['idReponse']?>_modifs'); out('<?=$dataReponse['idReponse']?>_lecture'); over('<?=$dataReponse['idReponse']?>_modifs_btn'); out('<?=$dataReponse['idReponse']?>_lecture_btn')"><i class="far fa-edit"></i> Modifier</a>
										<a class="supprimer" onclick="over('<?=$dataReponse['idReponse']?>_overlay_reponse')"><i class="fas fa-trash-alt"></i> Supprimer</a>
									</div>
									<div class="option" style="display: none;" id="<?=$dataReponse['idReponse']?>_modifs_btn">
										<a class="annuler" onclick="out('<?=$dataReponse['idReponse']?>_modifs'); over('<?=$dataReponse['idReponse']?>_lecture'); out('<?=$dataReponse['idReponse']?>_modifs_btn'); over('<?=$dataReponse['idReponse']?>_lecture_btn')"><i class="far fa-minus-square"></i> Annuler modification</a>
										<input type="submit" class="valider_modification" name="<?=$dataReponse['idReponse']?>_modification" value="Modifier">
									</div>
								<?php
								}
								?>
						...
						<div class="overlay_reponse" id="<?=$dataReponse['idReponse']?>_overlay_reponse">
							<div class="popup_reponse">
								<h2 class="popup_titre">Suppression d'une réponse</h2>
								<p class="popup_msg">Voulez-vous vraiment supprimer cette réponse ?</p>
								<div align="center">
									<form method="post">
										<input type="submit" name="<?=$dataReponse['idReponse']?>_supprimer_reponse" class="popup_lien" value="Supprimer">
									</form>
									<a class="popup_lien" onclick="out('<?=$dataReponse['idReponse']?>_overlay_reponse')">Annuler</a>
								</div>
							</div>
						</div>
		...
		<div class="repondre_style">
			<?php if(isset($_SESSION['role'])) {
				?>
				<a class="repondre" href="#nouvelle_reponse" onclick="over('nouvelle_reponse')">Répondre</a>
				<?php
			}
			else {
				?>
				<a class="repondre" href="DiagHealth_connexion.php">Répondre</a>
				<?php
			}
			?>
		</div>
	...
	<div class="retour_style">
		<a class="retour" href="DiagHealth_forum.php">Retour</a>
	</div>
	<div id="nouvelle_reponse" style="display: none;">
		...
		<form method="post">
			<table class="nouvelle_reponse_table">
				<tr class="nouvelle_reponse_tr">
					...
					<td>
						<textarea name="message" class="insert_rep" placeholder=" Insérez votre réponse" required></textarea>
					</td>
				</tr>
				<tr>
					<td><td align="right">
						<input type="submit" class="nouvelle_reponse_btn" name="nouvelle_reponse_btn" value="Répondre">
					</td>
				</tr>
			</table>
		</form>
		<a class="retour" onclick="out('nouvelle_reponse')">Annuler</a>
		...
		<?php if(isset($_SESSION['role'])) {
			if($_SESSION['role'] == 'Administrateur') { 
				?>
				<a class="suppression" onclick="over('overlay_sujet')"><i class="fas fa-trash-alt"></i> Supprimer les éléments</a>
				<?php
			}
		}
	?>
		...
		<?php
			...
			for ($k = 0; $k <= count($dataFavoris)-1; $k++) {
				...
				if($dataFavoris[$k][1] == $_SESSION['idUtilisateur'] && $dataFavoris[$k][2] == $_GET['id_sujet']) {
					?> 
					<div class="fav">
						<input type="submit" id="enlever_favoris" class="suivre" name="enlever_favoris" value="Ne plus suivre le sujet">
					</div>
					 <?php
				}
			}
			...
			if($increment_favoris == 0) {
				?>
				<div class="fav">
					 <input type="submit" id="ajouter_favoris" class="suivre" name="ajouter_favoris" value="Suivre le sujet">
				</div>
				 <?php
			}
		?>
