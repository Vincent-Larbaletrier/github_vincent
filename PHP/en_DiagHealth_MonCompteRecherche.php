...
if($_SESSION['connexion'] != '1' || $_SESSION['role'] == 'Utilisateur' || $_GET['id_user'] == '') {
	header('Location: en_DiagHealth_accueil.php');
}

else if($_SESSION['connexion'] == '1' && $_SESSION['role'] != 'Utilisateur' && $_GET['id_user'] != '') {
	...
	$rechercheinfo -> execute(array('idUtilisateur' => $_GET['id_user']));
	...
	if($infoCentre['ville'] == '') {
		$affichageCentre = "You do not belong to any center";
	} else {
	...
	if(isset($_FILES['pp']) AND !empty($_FILES['pp']['name'])) {
		...
		if($_FILES['pp']['size'] <= $taillemax) {
			...
			if(in_array($extU, $extV)) {
				$chemin = "users/pp/" .$_GET['id_user']. "." .$extU;
				...
				if($resultat) {
					...
					$updatepp -> execute(array(
						'pp' => $_GET['id_user'].'.'.$extU,
						'idUtilisateur' => $_GET['id_user']
					));
					...
					$msg = "An import error has occurred.";
				}
			}
			else {
				$msg = "Image is not in the correct format";
			}
		}
		else {
			$msg = "The image is too large.";
		}
	}
	...
	if(isset($_POST['validation'])) {
		...
		if(!empty($Nom) || !empty($Prenom)) {
			if(empty($Mdp_before) || empty($Mdp)) {
				$update_profil = 'UPDATE users SET nom="'.$Nom.'", prenom="'.$Prenom.'", centre="'.$Centre.'", dateNaissance="'.$Naissance.'", adresse="'.$Adresse.'", tel="'.$Tel.'" WHERE idUtilisateur="'.$_GET['id_user'].'"';
				...
				if($MdpBon) {
					$update_profil = 'UPDATE users SET nom="'.$Nom.'", prenom="'.$Prenom.'", mdp="'.$Mdp.'", centre="'.$Centre.'", dateNaissance="'.$Naissance.'", adresse="'.$Adresse.'", tel="'.$Tel.'" WHERE idUtilisateur="'.$_GET['id_user'].'"';
					$db-> exec($update_profil);
	...
	if(isset($_POST['supprim'])){
		$suppression_user = "DELETE FROM users where idUtilisateur='".$_GET['id_user']."'";
		$db -> exec($suppression_user);

		header('Location: en_DiagHealth_gestion_admin.php');
	}
	...
	<body>
		...
		<div id="photo" style="background-image: url(../Public/Images/fond0.jpg);" align="center">
			<div class="border" align="center">
				<img id="pp" src="users/pp/<?= $dataCompte['pp']; ?>">
			</div>
		</div>
		<div id="parcourir" align="center">
			<form method="post" enctype="multipart/form-data">
				<input type="file" class="modif_pp" name="pp" required> <br>
				<input type="submit" id="modif_pp" name="modif_pp" value="Change my profile picture">
			</form>
		</div>
		<h1> <?= $dataCompte['prenom']; ?>'s account / personal information </h1>
			...
			<form method="post">
				<table>
					<tr>
						...
						<td class="mod">
							<div id="nom_btn">
								<a onclick="over('nom_modif'); out('nom'); over('nom_annuler'); out('nom_btn');" class="bouton"><i class="far fa-edit"></i> Edit</a>
							</div>
							<div id="nom_annuler" style="display: none;">
								<a onclick="out('nom_modif'); over('nom'); out('nom_annuler'); over('nom_btn');" class="annuler_modif">Discard changes</a>
							</div>
						</td>
					</tr>
					<tr class="espace"></tr>
					<tr>
						...
						<td class="mod">
							<div id="prenom_btn">
								<a onclick="over('prenom_modif'); out('prenom'); over('prenom_annuler'); out('prenom_btn');" class="bouton"><i class="far fa-edit"></i> Edit</a>
							</div>
							<div id="prenom_annuler" style="display: none;">
								<a onclick="out('prenom_modif'); over('prenom'); out('prenom_annuler'); over('prenom_btn');" class="annuler_modif">Discard changes</a>
							</div>
						</td>
					</tr>
					<tr class="espace"></tr>
					...
					<tr>
						...
						<td>
							<div id="mdp"><?= "**********"; ?></div>
							<div id="mdp_conf" style="display: none;"><label>Enter your current password</label><input type="password" name="mdp_before"></div>
							<div id="mdp_modif" style="display: none;"><label>Enter the new password</label><input type="password" name="mdp"></div>
						</td>
						<td class="mod">
							<div id="mdp_btn">
								<a onclick="over('mdp_modif'); over('mdp_conf'); out('mdp'); over('mdp_annuler'); out('mdp_btn');" class="bouton"><i class="far fa-edit"></i> Edit</a>
							</div>
							<div id="mdp_annuler" style="display: none;">
								<a onclick="out('mdp_modif'); out('mdp_conf'); over('mdp'); out('mdp_annuler'); over('mdp_btn');" class="annuler_modif">Discard changes</a>
							</div>
						</td>
					</tr>
					<tr class="espace"></tr>
					<tr>
						...
						<td>
							...
							<div id="centre_modif" style="display: none;">
								<input type="radio" name="centre" value="0"><a class="lien_map">None</a><br>
								<?php 
						...
						<td class="mod">
							<div id="centre_btn">
								<a onclick="over('centre_modif'); out('centre'); over('centre_annuler'); out('centre_btn');" class="bouton"><i class="far fa-edit"></i> Edit</a>
							</div>
							<div id="centre_annuler" style="display: none;">
								<a onclick="out('centre_modif'); over('centre'); out('centre_annuler'); over('centre_btn');" class="annuler_modif">Discard changes</a>
							</div>
						</td>
					</tr>
					<tr class="espace"></tr>
					<tr>
						...
						<td class="mod">
							<div id="dateN_btn">
								<a onclick="over('dateN_modif'); out('dateN'); over('dateN_annuler'); out('dateN_btn');" class="bouton"><i class="far fa-edit"></i> Edit</a>
							</div>
							<div id="dateN_annuler" style="display: none;">
								<a onclick="out('dateN_modif'); over('dateN'); out('dateN_annuler'); over('dateN_btn');" class="annuler_modif">Discard changes</a>
							</div>
						</td>
					</tr>
					<tr class="espace"></tr>
					<tr>
						...
						<td class="mod">
							<div id="adresse_btn">
								<a onclick="over('adresse_modif'); out('adresse'); over('adresse_annuler'); out('adresse_btn');" class="bouton"><i class="far fa-edit"></i> Edit</a>
							</div>
							<div id="adresse_annuler" style="display: none;">
								<a onclick="out('adresse_modif'); over('adresse'); out('adresse_annuler'); over('adresse_btn');" class="annuler_modif">Discard changes</a>
							</div>
						</td>
					</tr>
					<tr class="espace"></tr>
					<tr>
						...
						<td class="mod">
							<div id="tel_btn">
								<a onclick="over('tel_modif'); out('tel'); over('tel_annuler'); out('tel_btn');" class="bouton"><i class="far fa-edit"></i> Edit</a>
							</div>
							<div id="tel_annuler" style="display: none;">
								<a onclick="out('tel_modif'); over('tel'); out('tel_annuler'); over('tel_btn');" class="annuler_modif">Discard changes</a>
							</div>
						</td>
					</tr>
					<tr class="espace"></tr>
					...
					<tr>
						...
						<td>
							...
							<div id="test_demande"><?php if($dataCompte['testDemande'] == 0) {echo "No pending test request";} else {echo "Test request pending on : " .$dataCompte['dateDemande'] .".";} ?></div>
						</td>
						<td class="mod"></td>
					</tr>
					<tr class="espace"></tr>
				...
				<div class="sauvegarder" align="right">
					<a class="save"><i class="far fa-save"></i><input type="submit" class="save_submit" name="validation" value="Save changes"></a>
				</div>
			...
			<?php if($_SESSION['role'] == 'Administrateur') {
				...
				<div id="bouton">
					<form id="ajouter" name="ajouter" method="post">
						<input type="submit" class="supprim" name="supprim" onclick="return confirmDelete()" value="Delete this user">
					</form>
				</div>
				<?php
			}
