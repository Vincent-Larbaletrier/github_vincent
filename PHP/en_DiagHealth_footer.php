<?php
	//session_start();
?>

<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Footer</title>
    <link rel="stylesheet" href="../Public/CSS/DiagHealth_footer.css">
    <link rel="stylesheet" href="../Public/CSS/DiagHealth_base.css">
</head>
<div class="espace"></div>
<footer>
    <div class="footer_logostart">
        <a href="en_DiagHealth_accueil.php"><img src="../Public/Images/logo.png" alt="" width="70px" /></img></a>
    </div>
    <div class="footer_social_icons">
        <ul>
            <li><a href="#" target="blanck"><img class="facebook"
                        src="data:image/png;base64," /></a>
            </li>
            <li><a href="#" target="blanck"><img
                        src="data:image/png;base64," /></a>
            </li>
            <li><a href="#" target="blanck"><img class="youtube"
                        src="data:image/png;base64," /></a>
            </li>
            <li><a href="#" target="blanck"><img class="linkedin"
                        src="data:image/png;base64," /></a>
            </li>
        </ul>
    </div>
    <div class="footer_menu_one">
        <ul>
            <li><a href="en_DiagHealth_accueil_plus.php"><i class="fas fa-info-circle"></i> About</a></li>
            <li><a href="en_DiagHealth_contact.php"><i class="fas fa-mail-bulk"></i> Contact</a></li>
            <li><a href="en_DiagHealth_CGU.php"><i class="far fa-copyright"></i> CGU-Legal notices</a></li>
            <li><a href="en_DiagHealth_faq.php"><i class="far fa-question-circle"></i> FAQ</a></li>
            <li><a href="en_DiagHealth_forum.php"><i class="far fa-comment-dots"></i> Forum</a></li>
        </ul>
    </div>
    <div class="footer_bottom">
        <p>Design by: <a href="#">DiagHealth</a></p>
    </div>
</footer>